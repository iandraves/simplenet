...
Image get_image_data(double imageArr[][784], int labelArr[], int index)
{
    Image img;

    img.image = imageArr[index];
    img.label = labelArr[index];

    return img;
}

int mount(Network network, int imageNumber)
{
    load_mnist();

    Image image = get_image_data(test_image, test_label, 0);

    Matrix input = network[0].activations;

    for (int i = 0; i < input.rows; i++)
        input.matrix[i][0] = (float)image.image[i];
